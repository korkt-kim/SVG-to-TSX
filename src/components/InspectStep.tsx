...
export const InspectStep = () => {
  ...
  return (
    <CommonLayout>
      ...
      {isSvgComponentSelected ? globalContext?.svgCodes.map((svg, index) => (
        <SVGItem key={index} name={svg.name} />
      )): <><p>No Component(or Instance) icons are selected.</p>
      <a target='_blank' href="https://help.figma.com/hc/en-us/articles/360038662654-Guide-to-components-in-Figma">What are Components and Instances?</a></>}
</CommonLayout.Content>
      <CommonLayout.Bottom>
      <button
        onClick={async () => {
