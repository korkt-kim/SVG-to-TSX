...
export const ExportStep = () => {
    ...
    return (
        ...
        <form id="export" style={{display:'flex', flexDirection:'column'}} onSubmit={(e)=>{
            e.preventDefault()
            console.log("rrrr")
            codeHostingRef.current?.createPR({
                ...
                svgs: globalContext?.tsxCodes.map(tsx => ({name: tsx.name, code: tsx.tsx})) || []
            })
        }}>
             
            <label htmlFor={InputIDs.accessToken}>Access Token</label>
            <input type="password" id={InputIDs.accessToken} onChange={(e)=>savePersistentValue(InputIDs.accessToken, e.target.value)} />
            <label htmlFor={InputIDs.repoUrl}>repo url</label>
            <input type="text" id={InputIDs.repoUrl} onChange={(e)=>savePersistentValue(InputIDs.repoUrl, e.target.value)} />
            <label htmlFor={InputIDs.featureBranch}>Feature branch</label>
            <input type="text" id={InputIDs.featureBranch} onChange={(e)=>savePersistentValue(InputIDs.featureBranch, e.target.value)} />
            <label htmlFor={InputIDs.iconDirectory}>Icon directory</label>
            <input type="text" id={InputIDs.iconDirectory} onChange={(e)=>savePersistentValue(InputIDs.iconDirectory, e.target.value)} />
        ...
        <CommonLayout.Bottom>
            <button type="submit" form="export">Export</button>
        </CommonLayout.Bottom>
        </CommonLayout>
    )
}