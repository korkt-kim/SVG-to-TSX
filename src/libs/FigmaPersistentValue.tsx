...
const loadPersistentValue = () => {
  parent.postMessage(
    { pluginMessage: { type: POST_MESSAGE_TYPE.LOAD_ALL } },
    "*",
  );
};

export const FigmaPersistentValueProvider = ({
  children,
}: {
  children: ReactNode;
}) => {
...
export const useFigmaPersistentValue = () => {
  ...
  if (!context) {
    throw new Error(
      "useFigmaPersistentValue must be used within a FigmaPersistentValueProvider",
    );
  }
